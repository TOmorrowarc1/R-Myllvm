...
TEST(ValueTest, BinaryOperatorTypeCheckTest) {
    ...
    // 异常情况：操作数类型不是整数
    EXPECT_THROW(BinaryOperator("result2", &i32_type, &ptr_val, &int32_val, "add"), std::runtime_error);
    EXPECT_THROW(BinaryOperator("result3", &i32_type, &int32_val, &ptr_val, "add"), std::runtime_error);
    
    // 异常情况：操作数类型与结果类型不匹配
    EXPECT_THROW(BinaryOperator("result4", &i8_type, &int32_val, &int32_val, "add"), std::runtime_error);
    EXPECT_THROW(BinaryOperator("result5", &i32_type, &int8_val, &int8_val, "add"), std::runtime_error);
}
...
TEST(ValueTest, UnaryOperatorTypeCheckTest) {
    ...
    // 异常情况：操作数类型不是整数
    EXPECT_THROW(UnaryOperator("result2", "neg", &i32_type, &ptr_val), std::runtime_error);
    
    // 异常情况：操作数类型与结果类型不匹配
    EXPECT_THROW(UnaryOperator("result3", "neg", &i8_type, &int32_val), std::runtime_error);
    EXPECT_THROW(UnaryOperator("result4", "neg", &i32_type, &int8_val), std::runtime_error);
}
...
TEST(ValueTest, StoreInstTypeCheckTest) {
    ...
    // 异常情况：指针类型不是指针
    EXPECT_THROW(StoreInst(&i32_type, &int32_val, &int32_val), std::runtime_error);
    
    // 异常情况：值类型与指定类型不匹配
    EXPECT_THROW(StoreInst(&i32_type, &alloca_ptr, &int8_val), std::runtime_error);
    EXPECT_THROW(StoreInst(&i8_type, &alloca_ptr, &int32_val), std::runtime_error);
}
...
TEST(ValueTest, ICmpInstTypeCheckTest) {
    ...
    EXPECT_NO_THROW(ICmpInst("result1", "eq", &i1_type, &int32_val1, &int32_val2));
    EXPECT_NO_THROW(ICmpInst("result2", "ne", &i1_type, &int8_val1, &int8_val2));
    
    // 异常情况：操作数类型不是整数
    EXPECT_THROW(ICmpInst("result3", "eq", &i1_type, &alloca_ptr, &int32_val1), std::runtime_error);
    EXPECT_THROW(ICmpInst("result4", "eq", &i1_type, &int32_val1, &alloca_ptr), std::runtime_error);
    
    // 异常情况：操作数类型不相同
    EXPECT_THROW(ICmpInst("result5", "eq", &i1_type, &int32_val1, &int8_val1), std::runtime_error);
    EXPECT_THROW(ICmpInst("result6", "eq", &i1_type, &int8_val1, &int32_val1), std::runtime_error);
    
    // 异常情况：结果类型不是Int1Type
    EXPECT_THROW(ICmpInst("result7", "eq", &i32_type, &int32_val1, &int32_val2), std::runtime_error);
    EXPECT_THROW(ICmpInst("result8", "eq", &i8_type, &int8_val1, &int8_val2), std::runtime_error);
}
...
TEST(ValueTest, ConBrInstTypeCheckTest) {
    ...
    // 异常情况：条件值不是i1类型
    EXPECT_THROW(ConBrInst(&i32_val, &then_block, &else_block), std::runtime_error);
    EXPECT_THROW(ConBrInst(&alloca_ptr, &then_block, &else_block), std::runtime_error);
}
...
TEST(ValueTest, CallInstTest) {
    ...
    EXPECT_TRUE(print_result.find("%result = call i32 @test_func") != std::string::npos);
...
TEST(ValueTest, GetElementPtrInstTest) {
    ...
    EXPECT_TRUE(print_result.find("%result = getelementptr i32") != std::string::npos);
    EXPECT_TRUE(print_result.find("%ptr") != std::string::npos);
...
TEST(ValueTest, GetElementPtrInstStructTest) {
    ...
    // 测试访问整个结构体
    GetElementPtrInst gep1("struct_ptr", &ptr_type, &struct_type, &alloca_ptr, indices1);
    ...
    EXPECT_TRUE(print_result1.find("%struct_ptr = getelementptr %struct.Person") != std::string::npos);
    EXPECT_TRUE(print_result1.find("%person_ptr") != std::string::npos);
    EXPECT_TRUE(print_result1.find("i32 0") != std::string::npos);
    
    // 测试访问结构体成员
    GetElementPtrInst gep2("member_ptr", &ptr_type, &struct_type, &alloca_ptr, indices2);
    ...
    EXPECT_TRUE(print_result2.find("%member_ptr = getelementptr %struct.Person") != std::string::npos);
...
TEST(ValueTest, GetElementPtrInstTypeCheckTest) {
    ...
    EXPECT_NO_THROW(GetElementPtrInst("result1", &ptr_type, &i32_type, &alloca_ptr, indices1));
    
    // 异常情况：指针类型不是指针
    std::vector<Value*> indices2 = {&i32_val, &i8_val};
    EXPECT_THROW(GetElementPtrInst("result2", &ptr_type, &i32_type, &i32_val, indices2), std::runtime_error);
    
    // 异常情况：索引不是整数类型
    std::vector<Value*> indices3 = {&alloca_ptr, &i8_val};
    EXPECT_THROW(GetElementPtrInst("result3", &ptr_type, &i32_type, &alloca_ptr, indices3), std::runtime_error);
    
    std::vector<Value*> indices4 = {&i32_val, &alloca_ptr};
    EXPECT_THROW(GetElementPtrInst("result4", &ptr_type, &i32_type, &alloca_ptr, indices4), std::runtime_error);
}
