...
TEST(TypeTest, StructTypeTest) {
  ...
  EXPECT_TRUE(print_def_result.find("%struct.MyStruct = type {") !=
              std::string::npos);
...
TEST(TypeTest, StructWithArrayFieldTest) {
  ...
  EXPECT_TRUE(print_def_result.find("%struct.StructWithArray = type {") !=
              std::string::npos);
  EXPECT_TRUE(print_def_result.find("i8") != std::string::npos);
...
TEST(TypeTest, StructWithNestedStructTest) {
  ...
  EXPECT_TRUE(print_def_result.find("%struct.OuterStruct = type {") !=
              std::string::npos);
  EXPECT_TRUE(print_def_result.find("i1") != std::string::npos);
  EXPECT_TRUE(print_def_result.find("%struct.InnerStruct") !=
              std::string::npos);
  EXPECT_TRUE(print_def_result.find("}") != std::string::npos);
  ...
  std::vector<Type *> elements_with_different_inner = {&i1_type,
                                                       &different_inner_struct};
...
TEST(TypeTest, ArrayOfStructsTest) {
  ...
  EXPECT_TRUE(container_print_def_result.find(
                  "%struct.ContainerStruct = type {") != std::string::npos);
  EXPECT_TRUE(container_print_def_result.find("i1") != std::string::npos);
  EXPECT_TRUE(container_print_def_result.find("[%struct.ElementStruct x 3]") !=
              std::string::npos);
...
TEST(TypeTest, ComplexFunctionTypeTest) {
  ...
  std::vector<Type *> params_with_different_struct = {
      &ptr_type, &different_struct, &i32_type};
  FunctionType func_with_different_struct(&void_type,
                                          params_with_different_struct);
