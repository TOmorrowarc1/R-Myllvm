...
TEST(ModuleTest, BasicFunctionalityTest) {
  ...
  // 空模块应该返回空字符串
  EXPECT_TRUE(print_result.empty());
}
...
TEST(ModuleTest, FunctionManagementTest) {
  ...
  Function *func4 =
      module.getOrCreateFunction("another_func", another_func_type);
...
TEST(ModuleTest, GlobalVariableManagementTest) {
  ...
  // 创建全局变量
  GlobalVariable *var1 =
      module.getOrCreateGlobalVariable("test_var", i32_type, false, init_value);
  ...
  GlobalVariable *var4 = module.getOrCreateGlobalVariable(
      "another_var", i32_type, true, another_init_value);
...
TEST(ModuleTest, PrintTest) {
  ...
  auto print_result = module.print();
  EXPECT_TRUE(print_result.find("declare") != std::string::npos);

  func->createBasicBlock("entry");
  // 测试打印功能
  print_result = module.print();
  EXPECT_TRUE(print_result.find("@test_var") != std::string::npos);
  EXPECT_TRUE(print_result.find("define") != std::string::npos);
  EXPECT_TRUE(print_result.find("@test_func") != std::string::npos);
  // 检查全局变量和函数之间是否有空行分隔
  size_t var_pos = print_result.find("@test_var");
  size_t func_pos = print_result.find("define");
  EXPECT_NE(var_pos, std::string::npos);
  EXPECT_NE(func_pos, std::string::npos);
  EXPECT_GT(func_pos, var_pos);
  // 检查全局变量和函数之间是否有空行
  if (var_pos < func_pos) {
    size_t next_line = print_result.find('\n', var_pos);
    if (next_line != std::string::npos && next_line < func_pos) {
      size_t line_after_var = print_result.find('\n', next_line + 1);
      if (line_after_var != std::string::npos && line_after_var < func_pos) {
        // 检查是否有空行（连续两个换行符）
        EXPECT_EQ(print_result.substr(next_line, 2), "\n\n");
      }
    }
  }
}
...
TEST(ModuleTest, ComplexScenarioTest) {
  ...
  // 创建多个全局变量
  ConstantInt *init_value1 =
      context.getIntConstant(static_cast<IntegerType *>(i32_type), 100);
  GlobalVariable *var1 =
      module.getOrCreateGlobalVariable("var1", i32_type, false, init_value1);

  ConstantInt *init_value2 =
      context.getIntConstant(static_cast<IntegerType *>(i8_type), 42);
  GlobalVariable *var2 =
      module.getOrCreateGlobalVariable("var2", i8_type, true, init_value2);
  ...
  // 检查全局变量和函数之间是否有空行分隔
  size_t var1_pos = print_result.find("@var1");
  size_t var2_pos = print_result.find("@var2");
  size_t func_pos = print_result.find("declare");
  EXPECT_NE(var1_pos, std::string::npos);
  EXPECT_NE(var2_pos, std::string::npos);
  EXPECT_NE(func_pos, std::string::npos);

  // 确保函数在全局变量之后
  EXPECT_GT(func_pos, var1_pos);
  EXPECT_GT(func_pos, var2_pos);

  // 检查全局变量和函数之间是否有空行
  size_t last_var_pos = std::max(var1_pos, var2_pos);
  if (last_var_pos < func_pos) {
    size_t next_line = print_result.find('\n', last_var_pos);
    if (next_line != std::string::npos && next_line < func_pos) {
      size_t line_after_var = print_result.find('\n', next_line + 1);
      if (line_after_var != std::string::npos && line_after_var < func_pos) {
